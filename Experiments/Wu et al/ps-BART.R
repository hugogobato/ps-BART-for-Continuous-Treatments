...
calculate_loc_Y <- function(C,D) {
  -10 - (2 * C$C1 + 2 * C$C2 + 3 * C$C3 - 1 * C$C4 + 2 * C$C5) - 
    D * (0.1 - 0.1 * C$C1 + 0.1 * C$C4 + 0.1 * C$C5 + 0.1 * C$C3^2) + 0.13 * D^3
}
...
column_means[1] <- X$C1[j]
column_means[2] <- X$C2[j]
column_means[3] <- X$C3[j]
column_means[4] <- X$C4[j]
column_means[5] <- X$C5[j]
column_means[6] <- X$C6[j]
column_means[7] <- X$pi_hats[j]
