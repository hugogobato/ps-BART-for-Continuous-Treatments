...
dbfit <- bart(X,D,X,verbose=F,ntree = 300)
...
bart_model <- bart(cbind(X,D),Y,cbind(X_mean,D),verbose=F, keeptrees = TRUE,,ntree = 100)
...
library(openxlsx)
file_path <- "ps_BART_Trees_300.xlsx"
...
saveWorkbook(wb, "ps_BART_Trees_300_CATE_full.xlsx", overwrite = TRUE)

# Started at 17h -> 17h40