...
y_hat_test <- predict(dbfit, X= cbind(X_mean[,1], X_mean[,2]), Z = D, propensity = X_mean[,3])
...
predicted_y_CATE <- predict(dbfit, X= cbind(X_mean[,1], X_mean[,2]), Z = D_range,propensity = X_mean[,3])
...
cat("Time taken to run the 2 simulations:", time_taken, units(time_taken), "\n")
